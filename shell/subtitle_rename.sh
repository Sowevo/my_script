...
# 将所有文件的重命名操作结果输出到控制台。
# eg: bash subtitle_rename.sh "/Users/xxx/path/メカクシティアクターズ" "*.ass" "目隐都市的演绎者 - S01E# - 第 @ 集.ass"
...
if [[ -z "$1" || -z "$2" || -z "$3" || ! "$3" =~ [#@] ]]; then
  ...
  echo ""
  echo "参数说明："
  echo "目录路径：要遍历的目录路径 eg:/a/b/c"
  # *.ass
  echo "过滤规则：用于过滤文件的通配符表达式  eg:*.ass"
  # 食戟之灵 - S01E# - 第@集.chi.zh-cn.ass
  echo "新文件名格式：新文件名的格式，必须包含#或@,表示要替换的数字编号,#补零2位,@不补零  eg:食戟之灵 - S01E# - 第@集.chi.zh-cn.ass"
  exit 1
fi
